...
contract Airdrop is ERC20, Ownable { 
  ...
  constructor() ERC20("airdrop Token"," ADT") {
        ...
        //transferOwnership( <insert address if you want to transfer> );
    }
  ...
  function burn(uint256 amount) external onlyOwner {
    ...
    function airdrop(address[] calldata addresses, uint256[] calldata tokenAmounts) external onlyOwner {
        require(addresses.length <= 100, "More than 100 wallets assigned");
        require(addresses.length == tokenAmounts.length, "List length mismatch");

        uint256 airdropTotal = 0;
        for(uint i = 0; i < addresses.length; i++){
            airdropTotal += tokenAmounts[i];
            _transfer(msg.sender, addresses[i], tokenAmounts[i]);
        }
        
        require(balanceOf(msg.sender) >= airdropTotal, "Token balance too low after transfers");
